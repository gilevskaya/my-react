...
export type Node = Element | TextElement;
...
function updateDom(
  ...
  Object.keys(prevProps)
    ...
    .forEach((name) => {
      // @ts-ignore: ok to not have a list of possible properties
      dom[name] = "";
    });
...
function render(element: Element, container) {
  wipRoot = {
    dom: container,
    props: {
      children: [element],
...
function reconcileChildren(wipFiber: Fiber, elements: Node[]) {
  let index = 0;
